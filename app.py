...
class TrackingUI:
                ...
                with st.expander(f"🚑 {patient.name} - {patient.condition}", expanded=True):
                    ...
                    with col2:
                        ...
                        if st.session_state.user['role'] == 'Admin':
                            ...
                            if st.button("🚀 Start Simulation", key=f"sim_{patient.patient_id}", use_container_width=True):
                                if ambulance and patient.referring_hospital_lat and patient.receiving_hospital_lat:
                                    ...
                                    thread.daemon = True
                                    thread.start()
                                    st.success("Simulation started! Ambulance is now moving.")
                                    st.rerun()
        else:
            st.info("No active patient transfers to track")
            
            # Demo section for admins to create sample transfers
            if st.session_state.user['role'] == 'Admin':
                st.markdown("---")
                st.subheader("🎮 Demo Setup")
                if st.button("Create Sample Active Transfer", use_container_width=True):
    ...
    def create_sample_transfer(self):
        """Create a sample active transfer for demonstration"""
        try:
            # Get a patient and ambulance
            patients = self.db.get_all_patients()
            ambulances = self.db.get_available_ambulances()
            
            if patients and ambulances:
                patient = patients[0]
                ambulance = ambulances[0]
                
                # Update patient status and assign ambulance
                patient.status = 'Transporting to Destination'
                ...
                ambulance.current_location = "En route to patient"
                ...
                st.success("Sample transfer created and simulation started!")
                st.rerun()
            else:
                st.warning("Need at least one patient and one available ambulance")
        except Exception as e:
            st.error(f"Error creating sample transfer: {e}")
